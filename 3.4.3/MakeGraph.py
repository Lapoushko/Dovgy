...
class MakeGraph:
    ...
    def __init__(self, profes, years, avgSalary, avgSalaryProf, countVacsYear,
        ...
        self.gropuGraph(ax1, "Уровень зарплат по годам", self.avgSalary, self.years,
                        self.avgSalaryProf, 'средняя з/п', f'з/п {self.profession}')
        self.gropuGraph(ax2, 'Количество вакансий по годам', self.countVacsYear, self.years,
                        self.countVacsYearProf, 'Количество вакансий',
                               f'Количество вакансий {self.profession}')
        plt.tight_layout()
        figure.savefig(nameFile)

    def gropuGraph(self, ax, title, valuesX1, valuesY1, valuesX2, labelX1, labelX2):
        ax.grid(axis='y')
